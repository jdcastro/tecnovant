...
def vista_reporte(report_id):
    ...
    if not check_resource_access(recommendation.lot.farm, claims):
    ...
    foliar_details = _parse_json(recommendation.foliar_analysis_details, {})
    soil_details = _parse_json(recommendation.soil_analysis_details, {})
    optimal_raw = _parse_json(recommendation.optimal_comparison, {})
    ...
    for k, v in optimal_raw.items():
        if k in soil_keys and k not in foliar_keys:
            optimalLevels["soil"][k] = v
        elif k in foliar_keys:
            optimalLevels["foliar"][k] = v
        else:
            optimalLevels["soil"][k] = v
    ...
    foliarChartData = []
    for key in foliar_keys:
        if key in analysisData["foliar"] and key in optimalLevels["foliar"]:
            opt = optimalLevels["foliar"][key]
            foliarChartData.append(
                {
                    "name": SYMBOL_MAP.get(key, key[:2].upper()),
    ...
    for key in soil_keys:
        if key in analysisData["soil"] and key in optimalLevels["soil"]:
            opt = optimalLevels["soil"][key]
    ...
    def _limiting_nutrient():
        ln = recommendation.limiting_nutrient_id
        if not ln:
            return None
        if ln in analysisData["foliar"] and ln in optimalLevels["foliar"]:
            val = analysisData["foliar"].get(ln)
            levels = optimalLevels["foliar"].get(ln)
            optimal_mid = (
                (levels.get("min", 0) + levels.get("max", 0)) / 2
                if levels.get("max") is not None
                else levels.get("ideal", 0)
            )
            percentage = (val / optimal_mid * 100) if optimal_mid else None
            return {
                "name": ln,
                "value": val,
        ...
        if ln in analysisData["soil"] and ln in optimalLevels["soil"]:
            val = analysisData["soil"].get(ln)
            levels = optimalLevels["soil"].get(ln)
            optimal_mid = (
                (levels.get("min", 0) + levels.get("max", 0)) / 2
                if levels.get("max") is not None
                else levels.get("ideal", 0)
        ...
        return {"name": ln, "percentage": None, "type": "unknown"}

    recommendations = (
        recommendation.automatic_recommendations.splitlines()
    ...
    return render_template(
        ...
        **context,
        ...
        limitingNutrient=_limiting_nutrient(),
